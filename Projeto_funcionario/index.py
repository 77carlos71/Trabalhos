...
def main(page: ft.Page):
    ...
    def generate_pdf(e):
        ...
        # Centralizando a hora na metade da página
        from datetime import datetime
        footer_text = f"Relatório gerado em: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
        
        # Calculando a posição horizontal para centralizar o rodapé
        footer_width = c.stringWidth(footer_text, "Helvetica-Oblique", 10)
        footer_x_position = (width - footer_width) / 2  # Centralizando horizontalmente
        
        # Calculando a posição vertical para colocar a hora no meio da página
        footer_y_position = height / 2  # Centralizando verticalmente

        c.setFont("Helvetica-Oblique", 10)
        c.drawString(footer_x_position, footer_y_position, footer_text)
